...
# 进入容器检查文件结构
echo "3. 检查容器内文件结构..."
docker exec ai-prompt-res sh -c "
echo '=== 根目录 ===' && ls -la /usr/share/nginx/html/
echo '=== assets目录 ===' && ls -la /usr/share/nginx/html/assets/ 2>/dev/null || echo 'assets目录不存在'
echo '=== 检查index.html内容 ===' && head -20 /usr/share/nginx/html/index.html
"
...
echo "测试CSS文件访问..."
docker exec ai-prompt-res sh -c "
if [ -f /usr/share/nginx/html/assets/index-*.css ]; then
    echo 'CSS文件存在:'
    ls -la /usr/share/nginx/html/assets/index-*.css
else
    echo 'CSS文件不存在'
fi
"
